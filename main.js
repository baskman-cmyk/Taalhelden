...
function recordEnglishAnswer(part,theme,correct,stars=0){
 ...
 if(correct){
  const earned=.1;
  state.points=Math.round((state.points+earned)*10)/10;
  state.rewardProgress=Math.round(((state.rewardProgress||0)+earned)*10)/10;
  aggregate.points=Math.round(((aggregate.points||0)+earned)*10)/10;
  while(state.rewardProgress>=REWARD_TARGET){state.rewardProgress=Math.round((state.rewardProgress-REWARD_TARGET)*10)/10;state.rewards++;reward();}
 }
 persist();
}
