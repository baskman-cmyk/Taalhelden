(function(){
  ...
  function renderShell(){
    root.innerHTML=`<div class="la-shell">
      ...
      <aside class="la-quiz">
        <div class="la-quiz-top"><div class="la-metrics"><div><small id="laQLabel"></small></div><div><small>✅ Goed</small><strong id="laGood">0</strong></div><div><small>⭐ Leesscore</small><strong id="laPoints">0</strong></div></div><div class="la-progress"><span id="laProgress"></span></div></div>
  ...
  function feedback(good,title,text){
    ...
    f.innerHTML=`<div><h4>${good?'✅':'❌'} ${esc(title)}</h4><p>${esc(text)}</p>${good?'<span class="la-point">+1 goed ⭐</span>':''}</div>`;
  }
  ...
  function nextQuestion(){
    ...
    if(state.question<s.questions.length-1){state.question++;renderQuestion();}
      ...
      root.querySelector('#laResultText').textContent=`Je had ${state.correct} van de ${s.questions.length} vragen goed. Je actieve oefentijd telt mee voor punten.`;
